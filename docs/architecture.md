...
| [categories.md](../references/categories.md) | v1 고정 7개 카테고리 정의와 각 카테고리의 렌즈 |
...
디스패처는 카테고리 토큰을 그대로 `panel-workflow.md`에 넘기므로 `SKILL.md` 본문을 특별히 건드릴 필요는 없습니다. 다만 일곱 곳을 **동시에** 고쳐야 패널·홈·인터뷰·스킬·README·상태줄이 일관됩니다.
...
7. `.github/scripts/validate-clones.ts`의 `FIXED_CATEGORIES` Set에 새 값 추가.
