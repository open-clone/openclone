...
Every clone is a **folder** containing a `persona.md` file and an optional `knowledge/` subdirectory. **One clone = one folder = one person.** A clone can belong to multiple categories.
...
<clone-root>/
└── <name>/
    ├── persona.md                    # the clone definition (this schema)
    └── knowledge/                    # optional
        └── YYYY-MM-DD-<topic>.md     # append-only, dated topic files
```

Two clone roots are checked, in this order:

```
~/.openclone/clones/<name>/           # user clones (writable)
${CLAUDE_PLUGIN_ROOT}/clones/<name>/  # built-in clones (shipped with the plugin, read-only)
```

- `<name>` is a lowercase slug, `[a-z0-9-]+`, globally unique within each root.
- No subdirectories under `<name>/` beyond `knowledge/`; storage is otherwise flat.
- If the same `<name>` exists in both roots, the user folder wins. To customize a built-in clone, copy the folder to `~/.openclone/clones/` and edit there — `/openclone:ingest` does this automatically (fork-on-write) when the active clone is built-in.
...
Flat, append-only, dated topic files:
