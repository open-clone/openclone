...
카테고리를 늘리려면 다음 다섯 곳을 동시에 수정해야 합니다. 반만 반영하면 패널·홈 패널·인터뷰가 깨집니다. 디스패처(`commands/openclone.md`)는 카테고리 토큰을 그대로 `panel-workflow.md`에 넘기므로 여기서는 추가 작업이 없습니다.
...
4. `skills/openclone/SKILL.md`에서 해당 카테고리를 인식하도록 수정
5. `README.md` 카테고리 표 갱신
6. `.github/scripts/validate-clones.ts`의 `FIXED_CATEGORIES` 배열 갱신

### 훅 편집

- `hooks/inject-active-clone.sh`를 편집한 뒤에는 Claude Code 재시작 후 `/openclone <clone>` → 실제 메시지 전송으로 재현하세요. Room 모드를 건드렸다면 `/openclone room <a> <b>` 후 일반 메시지로도 한 번 점검합니다.
