...
내장 클론은 `clones/<slug>/` 폴더 하나로 끝납니다. 스키마 전체는 [references/clone-schema.md](references/clone-schema.md), 검증 규칙은 [.github/scripts/validate-clones.ts](.github/scripts/validate-clones.ts)에 있습니다.

#### 1. 폴더와 persona.md 만들기

`<slug>`는 소문자·영숫자·하이픈만(`[a-z0-9-]+`) 허용하며, 폴더 이름과 `persona.md`의 `name` 필드가 반드시 같아야 합니다.

```bash
mkdir -p clones/hyun
```

아래 템플릿을 `clones/<slug>/persona.md`로 저장하면 CI 검증(`validate-clones.ts`)을 그대로 통과합니다. 필수 frontmatter 6개 + 본문 섹션 4개(`## Persona` → `## Speaking style` → `## Guidelines` → `## Background`)가 **순서대로** 있어야 합니다.

```markdown
---
name: hyun                         # 폴더 이름과 동일한 슬러그
display_name: 김현                  # 목록·패널에 표시될 이름
tagline: 백엔드 엔지니어·시스템 디자인    # 한 줄 소개 (80자 이하)
categories: [tech]                 # 고정 7개 중 1개 이상 (리스트)
...
여러 카테고리에 걸친 클론(예: `founder`이면서 `vc`)은 `## Category-specific framing` 섹션과 `primary_category` 키를 추가합니다. 실전 예시는 [clones/douglas/persona.md](clones/douglas/persona.md) 참고.
...
| `categories` | 필수 | 고정 7개 enum 중 1개 이상(리스트) | `validate-clones.ts:23`, `42–58` |
...
고정 카테고리 7개: `vc`, `tech`, `founder`, `expert`, `influencer`, `politician`, `celebrity`. 이 외의 값은 CI가 잡습니다.

#### 카테고리 선택 가이드

- **단일 카테고리** — 클론의 관점·전문성이 하나로 뚜렷할 때. 예: 순수 엔지니어 → `[tech]`.
...
- **카테고리 오타·대문자** — `VC`·`engineer`·`Founder` 모두 실패. 7개 enum은 **소문자 정확 일치**.
...
6. `scripts/statusline.sh`의 `role_label` case에 한국어 라벨 추가
7. `.github/scripts/validate-clones.ts`의 `FIXED_CATEGORIES` 배열 갱신
...
openclone은 코드와 클론 콘텐츠에 서로 다른 라이선스를 적용합니다(자세한 분리는 [README의 라이선스 섹션](README.md#라이선스) 참고). PR을 제출하면 다음에 동의함을 의미합니다.

- **소스 코드 변경(저장소 전반)** — [MIT 라이선스](LICENSE) 하에 본인의 기여를 공개
- **`clones/` 콘텐츠 변경** — [CC BY-NC-SA 4.0](clones/LICENSE) 하에 본인의 기여를 공개. 외부 자료를 인용한 경우 frontmatter `source_url`을 반드시 채우고, NoDerivs 라이선스 자료나 사적 비공개 자료는 포함하지 마세요. 자세한 정책은 [clones/NOTICE.md](clones/NOTICE.md).
